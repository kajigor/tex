...
To evaluate our functional conversion scheme, we implemented the proposed algorithm in \haskell \todo{(footnote)}.
...
The implementation of the functional conversion \todo{(footnote)} as well as the execution code \todo{(footnote)} can be found on github.
...
In the forward direction we run the query \lstinline{mul$^o$ n 10 q} with \lstinline{n} in the range from $100$ to $1000$, and the functional conversion was $2$ degrees of magnitude faster: $927ms$ vs $9.4ms$ for the largest \lstinline{n}.
...
Here performance improved 3 degrees of magnitude: from $24s$ to $1.7s$ for the largest \lstinline{n}.
...
It took almost $1450ms$ to find the first $7$ pairs of numbers \lstinline{q} and \lstinline{r} such that \lstinline{10 * q = r}, while \ocanren was able to execute the query in $0.74ms$ (see figure~\ref{fig:mult_modded}).
...
It is not clear if this heuristics is universal.

\input{fig/mulo_time.tex}
...
By picking other monads, we can achieve performance improvement: for example, using Maybe for subtraction reduces execution time by \todo{(number)}.
