...
Then the body of the relation is analyzed (see line~\ref{line:body} in figure~\ref{fig:modeInference}).
...
If no guard is present, then it searches for the first assignment (the function \lstinline{<|>} in line~\ref{line:alt} is responsible for this), then for the match.
...
We cannot judge the relation to be a generator solely by its mode: the addition relation in the mode \lstinline{add$^o$ x$^{g \rightarrow g}$ y$^{f \rightarrow g}$ z$^{f \rightarrow g}$} generates an infinite stream, while \lstinline{add$^o$ x$^{f \rightarrow g}$ y$^{f \rightarrow g}$ z$^{g \rightarrow g}$} does not.
