\section{Functional Conversion of \micro}
...
When the relational conversion is used to create a relation, the order of the subgoals only really suits the forward direction, whereas the relation is often not intended to be run in it (in this case, it is better to run the original function).
...
Notice that we allow for some variables in the right-hand side to be ground in matches, given that at least one of them is free.
