...
\begin{center}
  \begin{tikzpicture}[
    every node/.style = {
      ...
      \node[] (p_o) at ($(norm.west)+(-0.6,0)$) {f$^o$};
      \node[] (p_n) at ($(norm.west)!(mode.north)!(norm.east)$) {f$^n$};
      \node[] (p_m) at ($(mode.west)!(inter.north)!(mode.east)$) {f$^m$};
      \node[] (p_i) at ($(inter.west)!(target.north)!(inter.east)$) {f$^i$};
