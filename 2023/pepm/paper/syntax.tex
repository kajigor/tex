...
We use syntax notation that identifiers which start with the upper-case letters are constructors, while identifiers which start with the lower-case letters are variable names.
...
To execute a relation, one should provide a query to run, for example the query \lstinline{run q (add$^o$ q q (S (S O)))} finds a number which, doubled, is 2 in Peano representation, namely \lstinline{S O}.
