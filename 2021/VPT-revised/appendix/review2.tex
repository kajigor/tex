...
\todo{\emph{Contradictions between conjuncts lead to failures at runtime. By detecting contradictions at specialization-time, we can remove the failing computations from programs thus improving its performance. This is the motivation behind our heuristic.}}
...
\todo{\emph{Due to the use of interleaving, the search in \mk is complete. Thus the order in which subgoals are evaluated is irrelevant: different evaluation strategies influence the order of the answers, not which answers can be potentially computed. Unfolding a goal in isolation helps reveal }}

This therefore means
that the conservative approach is not able to perform the deforestation or tupling transformations, which is a
major problem.

\todo{\emph{This is a major problem indeed. We are currently working on mitigating it.}}
...
\todo{
\emph{
  We put the prolog programs for maxlenght and double appendo in gists. Each file contains an input program in prolog, a goal to specialize, and the result of transformation done by ECCE with the default control settings. We used the instance of ECCE available online here: \url{http://wyvern.cs.uni-duesseldorf.de/ecce/index.php}
}
\emph{
\begin{itemize}
...
\todo{\emph{Section~\ref{discussion} states that our method cannot perform deforestation and tupling for some programs, and ConsPD will not improve them.}}
