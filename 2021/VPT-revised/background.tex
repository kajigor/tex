...
\begin{enumerate}
  ...
  \item An execution of a relational call $R^{k_i}_i(t_1, \dots, t_{k_i})$ is done by first substituting the terms $t_j$ for the respective formal parameters and then running the resulting goal.
  ...
  \item Disjunction $g_1 \vee g_2$ applies both goals to the current substitution, incrementally switching evaluation steps between the subgoals until all results (if any) are found.
\end{enumerate}
