...
{\Large Евгений Мартын}
\end{center} 

\bigskip

\begin{enumerate}
  \item Построить детерминированный конечный автомат для языка $(ba|b)^∗ \mid (bb|a)^∗$
  ...
  \item Можно ли распознать язык $\{ a^n b c^n \mid n \geq 1 \} \cup \{ a^n d c^{2n} \mid n \geq 1 \}$ при помощи алгоритма LL(1)? Если можно --- привести таблицу анализатора и продемонстрировать успешный и неуспешный вывод на строках длины не меньше 7. Если нельзя --- обосновать.
\end{enumerate}
