...
\begin{frame}[fragile]
  \frametitle{Специализация для \miniKanren: полное раскрытие}
    ...
    \begin{minipage}[h]{0.85\textwidth}
      \begin{tikzpicture}[->,node distance=1cm, sibling distance=4cm, scale=0.8, every node/.style={scale=0.8}]
        \tikzstyle{conf}=[rectangle,draw, rounded corners=.8ex,align=center]
          \node[conf] (0) {\textit{Unfolding} \\ \lstinline{len$^o$ x n $\wedge$ len$^o$ y n}};
...
\begin{frame}[fragile]
  \frametitle{Специализация для \miniKanren: раннее расщепление}
    ...
    \begin{minipage}[h]{0.7\textwidth}
      \begin{tikzpicture}[->,node distance=1cm, sibling distance=4cm, scale=0.8, every node/.style={scale=0.8}, tips=proper]
        \tikzstyle{conf}=[rectangle,draw, rounded corners=.8ex,align=center]
        \node[conf] (0) {\textit{Split} \\ \lstinline{len$^o$ x n $\wedge$ len$^o$ y n}};
